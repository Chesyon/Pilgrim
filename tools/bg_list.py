...
class BGToCopy:
    def __init__(self, entry: BgListEntry, eu_index: int, added: bool):
        ...
        self.added = added # Was this index outside the vanilla list? (In other words, was this added onto the end of the list?)
...
def compare_base_to_mod(base: BgList, modified: BgList) -> list[BGToCopy]:
    ...
    for i in range(len(level_base)):
        ...
        if str(base_entry) != str(modified_entry): # There's probably a more efficient way to compare BgListEntries than this.
...
def find_na_indexes(bgs_to_copy: list[BGToCopy], eu: BgList, na: BgList):
    ...
    for bg_to_copy in bgs_to_copy:
        if not bg_to_copy.added:
            try:
                eu_str = str(eu_list[bg_to_copy.eu_index]) # Get the vanilla version of the entry at this index so we can search for it.
